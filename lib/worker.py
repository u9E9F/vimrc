...
class WorkerInterface(object):
    def load_conf(self, conf):
        ...
        self._verbose = conf[common_name.VIMERATOR_OPT_VERBOSE]
...
>>>>>>> 991a570b8867e6f4ef88066d7062d054e7327d5b
    ...
    def _uninstall(self):            
        ...
        self.log('remove: %s' % vimrc_default_path)
        os.remove(vimrc_default_path)
        if os.listdir(vimrc_default_dir_path) == []:
            self.log('remove directory: %s' % vimrc_default_dir_path)
            os.rmdir(vimrc_default_dir_path)

        if self.__is_symbolic_link:
            self.log('remove: %s' % '~/.vimrc')
...
class WorkerPreset(WorkerInterface):
    def _install(self):
        for v in os.walk(self._vimerator_preset_path):
            ...
            self.log('%s -> %s' % (src_path, dst_path))
...
class WorkerGeneralConf(WorkerInterface):
    ...
    def _install(self):
        ...
        if not os.path.exists(vimrcs_path):
            self.log('create: %s' % vimrcs_path)
            os.makedirs(vimrcs_path)

        for conf_name in self.__confs_to_be_loaded:
            src_path = _expand('%s/%s' % (self._vimerator_conf_path, conf_name))
            dst_path = _expand('%s/%s' % (vimrcs_path, conf_name))
...
class WorkerPlugin(WorkerInterface):
    ...
    def _install(self):
        ...
        if not os.path.exists(vimrc_plugin_dir_path):
            self.log('create: %s' % vimrc_plugin_dir_path)
    ...
    def _uninstall(self):            
        ...
        if os.listdir(vimrc_plugin_dir_path) == []:
            self.log('remove: %s' % vimrc_plugin_dir_path)
